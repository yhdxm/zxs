...
export async function saveWordProgress(word: string, p: WordProgress): Promise<void> {
  ...
  if (error) {
    ...
    console.error('[degreeService] upsert with dates failed:', error.message)
    const legacyRow = {
      user_id: userId,
      word,
      status: p.status,
    ...
    const { error: err2 } = await supabase.from('degree_word_progress').upsert(legacyRow, { onConflict: 'user_id,word' })
    if (err2) {
      console.error('[degreeService] legacy upsert failed:', err2.message)
      // 本地镜像已更新；入离线队列，网络恢复后自动补发。
      reli.enqueue({ table: 'degree_word_progress', type: 'upsert', row: legacyRow })
    }
  }
}
