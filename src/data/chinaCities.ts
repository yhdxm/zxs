// 全国城市清单（直辖市 + 地级市，按省级行政区分组）
// 用途：天气页城市选择器下拉项（可按省筛选、可搜索、可自定义输入）。
// 说明：仅含直辖市与地级市（自治州/地区/盟未列入，避免高德天气接口无法匹配）。
// 数据源：GB/T 2260 行政区划口径，城市名用于高德地理编码/Open-Meteo 检索。
...
export const CHINA_CITY_GROUPS: CityGroup[] = [
...
export const ALL_CITIES: string[] = CHINA_CITY_GROUPS.flatMap((g) => g.cities)
