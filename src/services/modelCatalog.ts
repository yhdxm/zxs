...
export interface CallableModel {
  ...
  /** 接入提示 */
  note?: string
  /** 免费额度（tokens），仅免费模型有值；用于模型中心剩余额度计算 */
  freeQuota?: number
  /** 免费额度有效期（YYYY-MM-DD），仅免费模型有值 */
  freeUntil?: string
}
...
// 阿里百炼免费模型由 scripts/bailian_models_input.txt + genBailianModels.mjs 生成，
// 统一带免费额度(freeQuota)与有效期(freeUntil)。详见 bailianModels.generated.ts。
import { BAILIAN_MODELS } from './bailianModels.generated'
...
export const CALLABLE_MODELS: CallableModel[] = [
  ...
  // ===== 阿里百炼（DashScope）=====
  // 模型清单由 scripts/bailian_models_input.txt + genBailianModels.mjs 生成，
  // 统一标记 isFree + 免费额度(freeQuota) + 有效期(freeUntil)。详见 bailianModels.generated.ts。
  ...BAILIAN_MODELS,
